...
const MobNavMenu = ({ className, isToggled, NavigateSection }: Props) => {
  ...
  return (
    <motion.div
      className={clsx(
        "absolute right-0 top-0 z-[999] h-full w-0 overflow-hidden bg-navMenuBg outline backdrop-blur-xl",
        `${className}`,
      )}
...
const MenuItems = ({ text, highlight = "white", OnClick }: TextProps) => {
  return (
    <>
      <motion.div variants={ItemVariants} className="relative w-max text-text">
        <ButtonWrapper OnClick={OnClick}>
          <HighlightedText text={text} highlight={highlight} />
        </ButtonWrapper>
        <hr className="absolute mb-4 mt-1 w-[120%] -translate-x-10 border-line" />
      </motion.div>
    </>
  );
};
